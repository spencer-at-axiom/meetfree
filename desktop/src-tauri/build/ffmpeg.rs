...
pub fn ensure_ffmpeg_binary() -> Result<(), String> {
    let target = std::env::var("TARGET")
        .or_else(|_| std::env::var("HOST"))
        .map_err(|_| "Neither TARGET nor HOST environment variable set".to_string())?;
    ...
    download_and_extract_ffmpeg(&target, &binary_path)?;

    if !verify_ffmpeg_binary(&binary_path) {
        return Err("Downloaded FFmpeg binary verification failed".to_string());
    }
...
fn get_ffmpeg_url_for_target(target: &str) -> Result<String, String> {
    let url = if target.contains("windows") {
        "https://github.com/spencer-at-axiom/ffmpeg-binaries/releases/download/0.0.1/ffmpeg-8.0.1-essentials_build.zip"
    } else if target.contains("apple") {
        if target.contains("aarch64") {
            "https://github.com/spencer-at-axiom/ffmpeg-binaries/releases/download/0.0.1/ffmpeg80arm.zip"
        } else {
            "https://github.com/spencer-at-axiom/ffmpeg-binaries/releases/download/0.0.1/ffmpeg-8.0.1.zip"
        }
    } else if target.contains("linux") {
        if target.contains("aarch64") || target.contains("arm") {
            "https://github.com/spencer-at-axiom/ffmpeg-binaries/releases/download/0.0.1/ffmpeg-release-arm64-static.tar.xz"
        } else {
            "https://github.com/spencer-at-axiom/ffmpeg-binaries/releases/download/0.0.1/ffmpeg-release-amd64-static.tar.xz"
        }
    } else {
