...
pub async fn get_groq_models<R: Runtime>(
    ...
    let api_key = match api_key {
        ...
        _ => SettingsRepository::get_api_key(state.db_manager.pool(), "groq")
            .await
            .map_err(|e| format!("Failed to access stored Groq credentials: {}", e))?,
    };

    // Return fallback if no API key is available
    let api_key = match api_key {
        Some(key) => key,
        None => {
            log::info!("No Groq API key available, returning fallback models");
