...
pub async fn complete_onboarding<R: Runtime>(
    ...
    let selected_summary_model = if model.trim().is_empty() {
    ...
    info!(
        "Completing onboarding with summary model: {}",
    ...
    // Onboarding stores the selected Ollama summary model.
    ...
    // Summary model availability is based on whether Ollama reports at least one local model.
    let summary_ready = crate::ollama::get_ollama_models(None)
        .await
    ...
    info!(
        "Onboarding completed successfully with model: {}",
        selected_summary_model
    );
    Ok(())
}
