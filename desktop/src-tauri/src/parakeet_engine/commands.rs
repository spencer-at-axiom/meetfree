...
fn recover_lock<'a, T>(mutex: &'a Mutex<T>, name: &str) -> MutexGuard<'a, T> {
    mutex.lock().unwrap_or_else(|poisoned| {
        log::warn!("Recovering from poisoned Parakeet command lock: {}", name);
        poisoned.into_inner()
    })
}
...
pub fn set_models_directory<R: Runtime>(app: &AppHandle<R>) {
    let app_data_dir = match app.path().app_data_dir() {
        Ok(path) => path,
        Err(error) => {
            log::error!("Failed to get app data dir for Parakeet models: {}", error);
            return;
        }
    };
    ...
    let mut guard = recover_lock(&MODELS_DIR, "models_dir");
    *guard = Some(models_dir);
}

/// Get the configured models directory
fn get_models_directory() -> Option<PathBuf> {
    recover_lock(&MODELS_DIR, "models_dir").clone()
}

#[command]
pub async fn parakeet_init() -> Result<(), String> {
    let mut guard = recover_lock(&PARAKEET_ENGINE, "parakeet_engine");
    if guard.is_some() {
        return Ok(());
    }
...
pub async fn parakeet_get_available_models() -> Result<Vec<ModelInfo>, String> {
    let engine = {
        let guard = recover_lock(&PARAKEET_ENGINE, "parakeet_engine");
        guard.as_ref().cloned()
    };
