...
pub fn create_tray<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    if let Some(icon) = app.default_window_icon() {
        ...
        log::warn!("Default window icon unavailable; creating tray without a custom icon");
    }
...
fn toggle_recording_handler<R: Runtime>(app: &AppHandle<R>) {
    ...
    tauri::async_runtime::spawn(async move {
        if crate::is_recording().await {
            ...
            let stop_result = crate::stop_and_finalize_recording(app_clone.clone()).await;
...
async fn get_current_recording_state<R: Runtime>(app: &AppHandle<R>) -> RecordingState {
    ...
    // Check if paused
    let is_paused = crate::audio::recording_commands::is_recording_paused(app.clone()).await;
