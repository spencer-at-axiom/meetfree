...
async fn ensure_notification_manager(
    app: AppHandle<Wry>,
    manager_state: &NotificationManagerState<Wry>,
) -> Result<(), String> {
    if manager_state.read().await.is_some() {
        return Ok(());
    }

    let manager = initialize_notification_manager(app)
        .await
        .map_err(|e| format!("Failed to initialize notification manager: {}", e))?;
...
pub async fn get_notification_settings(
    ...
    ensure_notification_manager(app, &manager_state).await?;
        ...
        match initialize_notification_manager(app_handle.clone()).await {
            ...
            Err(e) => {
                ...
                if !settings.consent_given || !settings.system_permission_granted {
                    log_info!(
                        "Notification consent or permission missing, skipping fallback notification"
                    );
                    return Ok(());
                }
                ...
                let title = crate::brand::APP_NAME;
...
pub async fn show_recording_stopped_notification<R: Runtime>(
    ...
    if let Some(manager) = manager_lock.as_ref() {
        ...
        if !settings.consent_given || !settings.system_permission_granted {
            log_info!("Notification consent or permission missing, skipping fallback");
            return Ok(());
        }
