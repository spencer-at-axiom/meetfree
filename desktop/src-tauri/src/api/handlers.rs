...
pub async fn api_get_model_config<R: Runtime>(
    ...
    match SettingsRepository::get_model_config(pool).await {
        Ok(Some(config)) => {
            log_info!(
                "Γ£à Found model config in database: provider={}, model={}, whisperModel={}, ollamaEndpoint={:?}",
                &config.provider,
                &config.model,
            ...
            match SettingsRepository::get_api_key(pool, &config.provider).await {
                Ok(api_key) => {
                    log_info!("Successfully retrieved model config metadata.");
                    Ok(Some(ModelConfig {
        ...
        Ok(None) => {
            log_warn!("ΓÜá∩╕Å No model config found in database - database may be empty or settings table not initialized");
            Ok(None)
        }
        Err(e) => {
            log_error!("Γ¥î Failed to get model config from database: {}", e);
            Err(e.to_string())
        }
    }
}
...
pub async fn api_save_model_config<R: Runtime>(
    ...
    log_info!(
        "≡ƒÆ╛ api_save_model_config called (native): provider='{}', model='{}', whisperModel='{}', ollamaEndpoint={:?}",
        &provider,
        &model,
        &whisper_model,
        &ollama_endpoint
    ...
    .await
    {
        log_error!("Γ¥î Failed to save model config to database: {}", e);
    ...
    if let Some(key) = api_key {
        if !key.is_empty() && provider != "custom-openai" {
            log_info!("≡ƒöæ API key provided, saving...");
            if let Err(e) = SettingsRepository::save_api_key(pool, &provider, &key).await {
                log_error!("Γ¥î Failed to save API key: {}", e);
                return Err(e.to_string());
            }
        }
    }

    // Local summary providers are handled via configured provider endpoints.

    log_info!("Γ£à Successfully saved model configuration to database");
    Ok(
...
pub async fn api_save_transcript_config<R: Runtime>(
    ...
    if !SettingsRepository::is_supported_transcript_provider(&provider) {
        return Err(format!(
            "Unsupported transcript provider '{}'. Supported providers: parakeet, localWhisper",
            provider
        ));
    }
    ...
    if api_key.as_ref().is_some_and(|key| !key.trim().is_empty()) {
        log_warn!(
            "Ignoring transcript API key for provider '{}' because active transcript providers are local-only",
            provider
        );
    }
...
pub async fn open_external_url(url: String) -> Result<(), String> {
    ...
    let is_allowed = match parsed.scheme() {
        "https" => true,
    ...
    if !is_allowed {
        return Err(
            "Blocked unsupported URL. Only HTTPS URLs are allowed, except local HTTP URLs."
                .to_string(),
        );
    }
...
pub async fn api_save_custom_openai_config<R: Runtime>(
    ...
    match SettingsRepository::save_custom_openai_config(pool, &config).await {
        Ok(()) => {
            log_info!(
                "Γ£à Successfully saved custom OpenAI config for endpoint: {}",
                config.endpoint
            );
            Ok(serde_json::json!({
                "status": "success",
        ...
        Err(e) => {
            log_error!("Γ¥î Failed to save custom OpenAI config: {}", e);
    ...
    match SettingsRepository::get_custom_openai_config(pool).await {
        Ok(config) => {
            if let Some(ref c) = config {
                log_info!(
                    "Γ£à Found custom OpenAI config: endpoint='{}', model='{}'",
                    c.endpoint,
                    c.model
                );
            } else {
            ...
            Ok(config.map(|config| CustomOpenAIConfigView {
                ...
                top_p: config.top_p,
                has_stored_api_key: config.api_key.is_some(),
            }))
        }
        Err(e) => {
            log_error!("Γ¥î Failed to get custom OpenAI config: {}", e);
...
pub async fn api_test_custom_openai_connection<R: Runtime>(
    ...
    let resolved_api_key = match api_key.filter(|k| !k.trim().is_empty()) {
        Some(key) => Some(key),
        None => SettingsRepository::get_custom_openai_config(state.db_manager.pool())
            .await
            .map_err(|e| format!("Failed to load custom OpenAI configuration: {}", e))?
    ...
    match request.send().await {
            ...
            if status.is_success() {
                ...
                match serde_json::from_str::<serde_json::Value>(&response_text) {
                    Ok(json) => {
                                ...
                                if !choices_array.is_empty() {
                                    ...
                                    if let Some(first_choice) = choices_array.first() {
                                        ...
                                        if has_message_structure {
                                            log_info!("Γ£à Custom OpenAI connection test successful - response validated");
                        ...
                        log_warn!(
                            "ΓÜá∩╕Å Endpoint returned 200 but response doesn't match OpenAI format: {}",
                            response_text
                        );
                    ...
                    Err(e) => {
                        log_warn!(
                            "ΓÜá∩╕Å Endpoint returned 200 but response is not valid JSON: {}",
                            e
                        );
                        Err(format!(
                ...
                log_warn!(
                    "ΓÜá∩╕Å Custom OpenAI connection test failed with status {}: {}",
                    status,
                    response_text
                );
                Err(format!(
        ...
        Err(e) => {
            log_error!("Γ¥î Custom OpenAI connection test failed: {}", e);
            if e.is_timeout() {
