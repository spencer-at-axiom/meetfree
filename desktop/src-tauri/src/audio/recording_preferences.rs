...
fn normalize_saved_device_name(device_name: Option<String>) -> Option<String> {
    let raw = device_name?;
...
fn normalize_recording_preferences(mut preferences: RecordingPreferences) -> RecordingPreferences {
    preferences.preferred_mic_device =
        normalize_saved_device_name(preferences.preferred_mic_device);
    preferences.preferred_system_device =
        normalize_saved_device_name(preferences.preferred_system_device);
    preferences
}
    ...
    let prefs = if let Some(value) = store.get("preferences") {
        match serde_json::from_value::<RecordingPreferences>(value.clone()) {
            Ok(p) => {
                ...
                normalize_recording_preferences(p)
            }
            Err(e) => {
...
pub async fn save_recording_preferences<R: Runtime>(
    ...
    let preferences = normalize_recording_preferences(preferences.clone());
...
pub async fn get_available_audio_backends() -> Result<Vec<String>, String> {
    ...
    {
        // Non-macOS returns the compatibility backend label.
