...
use super::pipeline::{AudioPipelineConfig, AudioPipelineManager, PipelineDeviceProfile};
...
pub struct RecordingManager {
    ...
    pipeline_sender: Option<mpsc::UnboundedSender<AudioChunk>>,
    reconnecting: bool,
    disconnected_device: Option<(String, RecordingDeviceType)>,
}
...
impl RecordingManager {
    ...
    pub async fn start_recording(
        ...
        // Create the recording sender for pre-mixed pipeline audio.
        // The pipeline mixes mic and system audio before sending chunks here.
        ...
        let pipeline_handles = self.pipeline_manager.start(
            transcription_sender,
            AudioPipelineConfig {
                target_chunk_duration_ms: 0, // Ignored - using dynamic sizing internally
                sample_rate: 48000,          // 48kHz sample rate
                recording_sender: Some(recording_sender),
                device_profile: PipelineDeviceProfile {
                    mic_device_name: mic_name,
                    mic_device_kind: mic_kind,
                    system_device_name: sys_name,
                    system_device_kind: sys_kind,
                },
            },
        )?;
        ...
        let monitor_mic = microphone_device.clone();
        let monitor_sys = system_device.clone();
        ...
        self.stream_manager
            .start_streams(
                ...
                system_device.clone(),
                pipeline_handles.pipeline_sender.clone(),
            )
            .await?;

        self.pipeline_sender = Some(pipeline_handles.pipeline_sender);
        ...
        if let Some(ref mut monitor) = self.device_monitor {
            if let Err(e) = monitor.start_monitoring(monitor_mic, monitor_sys) {
    ...
    /// This prevents the Core Audio tap path from delivering variable sample rate
    ...
    /// - Speaker: If Bluetooth -> Use built-in MacBook speaker for the Core Audio tap path
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if microphone_device.is_none() {
                return Err(anyhow::anyhow!(
                    "âŒ No microphone device available for recording"
                ));
            }
    ...
    pub async fn stop_streams_only(&mut self) -> Result<()> {
        ...
        self.pipeline_sender = None;
        self.microphone_device = None;
        self.system_device = None;
        self.reconnecting = false;
        self.disconnected_device = None;
        ...
        if let Some(device) = device {
            ...
            match device_type {
                DeviceMonitorType::Microphone => {
                    ...
                    let system_device = self.system_device.clone();
                    let pipeline_sender = self
                        .pipeline_sender
                        .clone()
                        .ok_or_else(|| anyhow::anyhow!("Pipeline sender not available"))?;
                    ...
                    self.stream_manager
                        .start_streams(Some(device_arc.clone()), system_device, pipeline_sender)
                        .await?;
                    self.microphone_device = Some(device_arc);
                ...
                DeviceMonitorType::SystemAudio => {
                    ...
                    let microphone_device = self.microphone_device.clone();
                    let pipeline_sender = self
                        .pipeline_sender
                        .clone()
                        .ok_or_else(|| anyhow::anyhow!("Pipeline sender not available"))?;
                    ...
                    self.stream_manager
                        .start_streams(microphone_device, Some(device_arc.clone()), pipeline_sender)
                        .await?;
                    self.system_device = Some(device_arc);
            ...
            warn!("âŒ Device '{}' not yet available", device_name);
            Ok(false)
        }
    }
    ...
    /// Get disconnected device metadata for UI status.
    pub fn get_disconnected_device(&self) -> Option<(String, RecordingDeviceType)> {
        self.disconnected_device.clone()
    }
