    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        let (text, conf) = if use_parakeet {
            let engine = parakeet_engine
                .as_ref()
                .ok_or_else(|| anyhow!("Parakeet engine unavailable during retranscription"))?;
            let text = engine
            ...
            let engine = whisper_engine
                .as_ref()
                .ok_or_else(|| anyhow!("Whisper engine unavailable during retranscription"))?;
...
async fn get_or_init_whisper<R: Runtime>(
    ...
    let engine = {
        let guard = WHISPER_ENGINE.lock().unwrap_or_else(|poisoned| {
            warn!("Recovering from poisoned Whisper engine lock during retranscription");
            poisoned.into_inner()
        });
        guard.as_ref().cloned()
    };
...
async fn get_or_init_parakeet<R: Runtime>(
    ...
    let engine = {
        let guard = PARAKEET_ENGINE.lock().unwrap_or_else(|poisoned| {
            warn!("Recovering from poisoned Parakeet engine lock during retranscription");
            poisoned.into_inner()
        });
        guard.as_ref().cloned()
    };
