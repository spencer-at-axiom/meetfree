...
// `get_transcript_history` and `get_recording_meeting_name` now live in `recording_commands.rs`.
...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        let status_tracker = super::status::get_global_status();
        ...
        let transcription_engine = match super::engine::get_or_init_transcription_engine(&app).await
        {
            ...
            Err(e) => {
                ...
                status_tracker.stop_processing();
        ...
        // Use global status tracker's atomic counters
        let chunks_queued = status_tracker.get_arc_queued();
        let chunks_completed = status_tracker.get_arc_completed();
                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                                ...
                                .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    if !transcript.trim().is_empty() && meets_threshold {
                                        // Log transcription results, not every processing step.
                                        info!("✅ Worker {} transcribed: {} (confidence: {}, partial: {})",
                                              worker_id, transcript, confidence_str, is_partial);

                                        // Emit speech-detected once per session for frontend feedback.
                                        let current_flag =
                                            SPEECH_DETECTED_EMITTED.load(Ordering::SeqCst);
                                        ...
                                        // Skip per-emission logging to keep the hot path light.
                                    } else if !transcript.trim().is_empty() && should_log_this_chunk
                                    {
                                        // Log low-confidence results only occasionally.
                            ...
                            // Log progress every 5th chunk to keep I/O low.
