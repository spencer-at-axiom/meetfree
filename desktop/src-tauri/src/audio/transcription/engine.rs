...
fn recover_engine_lock<'a, T>(mutex: &'a Mutex<T>, name: &str) -> MutexGuard<'a, T> {
    mutex.lock().unwrap_or_else(|poisoned| {
        warn!(
            "Recovering from poisoned transcription engine lock: {}",
            name
        );
        poisoned.into_inner()
    })
...
/// Check whether the configured transcription provider has at least one usable model
/// without forcing model load into memory. This is used for passive readiness polling.
pub async fn check_transcription_model_available<R: Runtime>(
    app: &AppHandle<R>,
) -> Result<(), String> {
    let config =
        match crate::api::api_get_transcript_config(app.clone(), app.clone().state(), None).await {
            Ok(Some(config)) => config,
            Ok(None) => crate::api::TranscriptConfig {
                provider: "parakeet".to_string(),
                model: crate::config::DEFAULT_PARAKEET_MODEL.to_string(),
                has_stored_key: false,
            },
            Err(_) => crate::api::TranscriptConfig {
                provider: "parakeet".to_string(),
                model: crate::config::DEFAULT_PARAKEET_MODEL.to_string(),
                has_stored_key: false,
            },
    ...
    match config.provider.as_str() {
        "localWhisper" => {
            crate::whisper_engine::commands::whisper_init()
                .await
                .map_err(|e| format!("Failed to initialize speech recognition: {}", e))?;
            ...
            if models.iter().any(|model| {
                ...
                return Err("Transcription model download in progress".to_string());
            }

            if models
                .iter()
                .any(|model| matches!(model.status, crate::whisper_engine::ModelStatus::Available))
            {
                Ok(())
            } else {
                Err("No Whisper models are available. Please download a model to enable transcription.".to_string())
            }
        }
        "parakeet" => {
            crate::parakeet_engine::commands::parakeet_init()
                .await
                .map_err(|e| format!("Failed to initialize Parakeet speech recognition: {}", e))?;
                ...
                .any(|model| matches!(model.status, crate::parakeet_engine::ModelStatus::Available))
            {
                ...
                Err("No Parakeet models are available. Please download a model to enable transcription.".to_string())
            }
        }
        other => Err(format!(
            "Provider '{}' is not supported for local transcription. Please select 'localWhisper' or 'parakeet'.",
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        other => Err(format!(
            "Unsupported transcription provider '{}'. Please select 'parakeet' or 'localWhisper'.",
            other
        )),
    }
}
...
pub async fn get_or_init_whisper<R: Runtime>(
    ...
    let existing_engine = {
        let engine_guard = recover_engine_lock(
    ...
    if let Some(engine) = existing_engine {
        ...
        if engine.is_model_loaded().await {
            ...
            // Check whether the loaded model matches the saved config.
