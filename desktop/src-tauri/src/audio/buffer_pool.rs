...
fn recover_pool_lock<'a>(
    lock_result: std::sync::LockResult<std::sync::MutexGuard<'a, VecDeque<Vec<f32>>>>,
    context: &str,
) -> std::sync::MutexGuard<'a, VecDeque<Vec<f32>>> {
    lock_result.unwrap_or_else(|poisoned| {
        log::warn!(
            "Recovering from poisoned audio buffer pool lock during {}",
            context
        );
        poisoned.into_inner()
    })
...
impl AudioBufferPool {
    ...
    pub fn get_buffer(&self) -> Vec<f32> {
        let mut pool = recover_pool_lock(self.pool.lock(), "buffer checkout");
    ...
    pub fn return_buffer(&self, mut buffer: Vec<f32>) {
        ...
        let mut pool = recover_pool_lock(self.pool.lock(), "buffer return");
    ...
    pub fn pool_size(&self) -> usize {
        recover_pool_lock(self.pool.lock(), "pool size inspection").len()
    }

    /// Clear all buffers in the pool
    pub fn clear(&self) {
        recover_pool_lock(self.pool.lock(), "pool clear").clear();
    }
}
...
impl PooledBuffer {
    ...
    /// Mutable view of pooled sample storage (grows the backing `Vec` if needed).
    pub fn samples_mut(&mut self) -> &mut Vec<f32> {
        self.buffer
            .get_or_insert_with(|| Vec::with_capacity(self.pool.buffer_capacity))
    }

    fn backing_vec(&self) -> &Vec<f32> {
        static EMPTY_BUFFER: once_cell::sync::Lazy<Vec<f32>> = once_cell::sync::Lazy::new(Vec::new);

        self.buffer.as_ref().unwrap_or(&EMPTY_BUFFER)
    }

    /// Read-only view of pooled samples (empty slice when no buffer is checked out yet).
    pub fn samples(&self) -> &[f32] {
        self.backing_vec().as_slice()
    }
