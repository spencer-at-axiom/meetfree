...
pub struct TranscriptSegment {
    ...
    pub speaker: Option<String>, // 'mic' for microphone, 'system' for system audio
}
...
impl RecordingSaver {
    ...
    pub fn add_transcript_segment(&self, segment: TranscriptSegment) {
        ...
        // Persist incremental transcript updates to disk.
        if let Some(folder) = &self.meeting_folder {
    ...
    fn initialize_meeting_folder(
        ...
        if create_checkpoints {
            ...
            info!(
                "âœ… Incremental audio saver initialized for meeting: {}",
                meeting_name
            );
        } else {
    ...
    fn write_transcripts_json(&self, folder: &Path) -> Result<()> {
        ...
        info!(
            "âœ… Successfully wrote transcripts.json with {} segments",
            segments_clone.len()
        );
        Ok(())
    }
    ...
    pub async fn stop_and_save<R: Runtime>(
        ...
        if let Some(folder) = &self.meeting_folder {
            ...
            if !transcript_path.exists() {
                error!(
                    "âŒ Transcript file was not created at: {}",
                    transcript_path.display()
                );
                return Err("Transcript file verification failed".to_string());
            }
            info!(
                "âœ… Transcripts saved and verified at: {}",
                transcript_path.display()
            );
        }
        ...
        if let (Some(folder), Some(mut metadata)) = (&self.meeting_folder, self.metadata.clone()) {
            ...
            info!(
                "âœ… Metadata updated with duration: {:?}s",
