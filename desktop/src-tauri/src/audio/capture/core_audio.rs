...
#[cfg(target_os = "macos")]
fn recover_waker_lock<'a, T>(mutex: &'a Mutex<T>, name: &str) -> MutexGuard<'a, T> {
    mutex.lock().unwrap_or_else(|poisoned| {
        warn!("Recovering from poisoned CoreAudio lock: {}", name);
        poisoned.into_inner()
    })
}
...
impl CoreAudioCapture {
    ...
    pub fn new() -> Result<Self> {
        ...
        let tap_uid_ref = tap.uid().map_err(|e| {
            error!("❌ CoreAudio: Failed to get tap UID: {:?}", e);
            anyhow::anyhow!("Failed to get tap UID: {:?}", e)
        })?;
        let sub_tap = cf::DictionaryOf::with_keys_values(
            &[ca::sub_device_keys::uid()],
            &[tap_uid_ref.as_type_ref()],
        );

        // Create the aggregate device descriptor using only the tap to avoid duplicate capture.
        let agg_desc = cf::DictionaryOf::with_keys_values(
            ...
            &[
                ...
                cf::str!(c"meetfree-audio-tap").as_type_ref(),
                &output_uid,
    ...
    fn start_device(
        ...
        extern "C" fn audio_proc(
            ...
            let Some(ctx) = ctx else {
                error!("❌ CoreAudio: audio callback invoked without context");
...
fn process_audio_data(ctx: &mut AudioContext, data: &[f32]) {
    ...
    if pushed > 0 {
        let should_wake = {
            let mut waker_state = recover_waker_lock(&ctx.waker_state, "callback_waker_state");
...
impl Stream for CoreAudioStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        {
            let mut state = recover_waker_lock(&self.waker_state, "stream_waker_state");
