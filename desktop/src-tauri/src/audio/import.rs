    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        let (text, conf) = if use_parakeet {
            let engine = parakeet_engine
                .as_ref()
                .ok_or_else(|| anyhow!("Parakeet engine unavailable during import"))?;
            let text = engine
            ...
            let engine = whisper_engine
                .as_ref()
                .ok_or_else(|| anyhow!("Whisper engine unavailable during import"))?;
...
async fn get_or_init_whisper<R: Runtime>(
    ...
    let engine = {
        let guard = WHISPER_ENGINE.lock().unwrap_or_else(|poisoned| {
            warn!("Recovering from poisoned Whisper engine lock during import");
            poisoned.into_inner()
        });
        guard.as_ref().cloned()
    };
...
async fn get_or_init_parakeet<R: Runtime>(
    ...
    let engine = {
        let guard = PARAKEET_ENGINE.lock().unwrap_or_else(|poisoned| {
            warn!("Recovering from poisoned Parakeet engine lock during import");
            poisoned.into_inner()
        });
        guard.as_ref().cloned()
    };
...
pub async fn select_and_validate_audio_command<R: Runtime>(
    ...
    let extensions = AUDIO_EXTENSIONS.to_vec();
...
mod tests {
    ...
    fn test_split_segment_at_silence_splits_long_segment() {
        ...
        for s in samples.iter_mut().skip(25 * 16000).take(3200) {
            *s = 0.0;
        }
        let segment = crate::audio::vad::SpeechSegment {
            samples,
