...
impl NoiseSuppressionProcessor {
    ...
    ///
    /// Always returns the same length as the input to avoid latency drift.
    ///
    /// # Arguments
...
pub fn spectral_subtraction(audio: &[f32], d: f32) -> Result<Vec<f32>> {
    ...
    // Handle inputs longer than the FFT window.
    if audio.is_empty() {
        return Ok(Vec::new());
    }
...
// Estimate background noise with a simple average of sample magnitudes.
...
pub fn write_audio_to_file_with_meeting_name(
    ...
    let file_path = final_output_path.join(format!("{}_{}.mp4", sanitized_device_name, timestamp));
    // Run FFmpeg in a separate task
    if !skip_encoding {
        encode_single_audio(bytemuck::cast_slice(audio), sample_rate, 1, &file_path)?;
    }
...
mod tests {
    ...
    fn test_sanitize_filename_removes_invalid_chars() {
        assert_eq!(sanitize_filename("meeting.txt"), "meeting.txt");
        ...
        assert_eq!(sanitize_filename("meeting?notes"), "meeting_notes");
        assert_eq!(sanitize_filename("meeting\"notes"), "meeting_notes");
        assert_eq!(sanitize_filename("meeting<notes>"), "meeting_notes_");
        assert_eq!(sanitize_filename("meeting|notes"), "meeting_notes");
    }

    #[test]
    fn test_sanitize_filename_preserves_valid_chars() {
        assert_eq!(sanitize_filename("meeting-2024"), "meeting-2024");
        assert_eq!(sanitize_filename("meeting_notes"), "meeting_notes");
        assert_eq!(sanitize_filename("meeting (1)"), "meeting (1)");
    }

    #[test]
    fn test_normalize_v2_empty() {
        let input: Vec<f32> = vec![];
        let output = normalize_v2(&input);
        assert_eq!(output.len(), 0);
    }

    #[test]
    fn test_normalize_v2_single_sample() {
        let input = vec![0.5];
        let output = normalize_v2(&input);
    ...
    #[test]
    fn test_normalize_v2_preserves_length() {
    ...
    #[test]
    fn test_normalize_v2_silent_audio() {
    ...
    fn test_high_pass_filter_creation() {
        let filter = HighPassFilter::new(48000, 80.0);
        assert_eq!(filter.sample_rate, 48000.0);
        assert_eq!(filter.cutoff_hz, 80.0);
    }

    #[test]
    fn test_high_pass_filter_process_empty() {
    ...
    #[test]
    fn test_high_pass_filter_process_preserves_length() {
        let mut filter = HighPassFilter::new(48000, 80.0);
        let input: Vec<f32> = (0..1000).map(|i| (i as f32) * 0.001).collect();
