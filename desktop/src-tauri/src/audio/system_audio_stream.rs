    ...
    pub async fn start_streams(
        ...
        if let Some(sys_device) = system_device {
            ...
            if should_use_enhanced_system_audio(&sys_device) {
                ...
                info!("Falling back to the default system-audio path for: {}", sys_device.name);
                // Fallback to the default system-audio stream path
                ...
                warn!("Fallback system-audio stream created but not stored in enhanced manager");
            }
        }
