...
/// - System audio capture using the Core Audio tap path on macOS with consistent sample rates
...
///
/// Note: On macOS, system audio is captured from the output device through
/// the Core Audio tap path. Using built-in speakers helps keep sample rates
/// stable for reliable mixing with microphone.
///
...
pub fn find_builtin_output_device() -> Result<Option<AudioDevice>> {
        ...
        if let Ok(name) = device.name() {
            ...
            for pattern in &builtin_patterns {
                if name_lower.contains(pattern) {
                    ...
                    // (we want real hardware speakers for the Core Audio tap path)
                    ...
                    info!("ðŸ”Š Found built-in speaker: '{}'", name);
    ...
    warn!(
        "âš ï¸ No built-in speaker found (searched {} patterns)",
