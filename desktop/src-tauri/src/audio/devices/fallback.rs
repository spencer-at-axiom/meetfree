...
// - Core Audio tap capturing Bluetooth-processed streams with variable timing
//
// Strategy (macOS-only):
...
// captures via stable wired path (built-in mic + Core Audio tap on the selected output).
...
/// and the Bluetooth stack may resample dynamically. When the Core Audio tap
...
pub fn get_safe_recording_devices_macos() -> Result<(Option<AudioDevice>, Option<AudioDevice>)> {
    ...
    let final_mic = if let Some(ref mic) = default_mic {
        // Detect whether the microphone is Bluetooth.
        // Use a fixed probe size; detection relies mostly on device name and Core Audio metadata.
    ...
    // CRITICAL: On macOS, the Core Audio tap captures the digital audio stream being
    ...
    // - The Core Audio tap captures from that active output stream (pristine quality)
    // - We MUST keep the Bluetooth speaker as the system device so the tap
    ...
    // the tap will try to capture from built-in, but NO AUDIO IS THERE!
    let final_speaker = if let Some(ref speaker) = default_speaker {
        ...
        if device_kind.is_bluetooth() {
            ...
            info!("   macOS: Core Audio tap captures digital stream BEFORE Bluetooth encoding");
    ...
    match (&final_mic, &final_speaker) {
        (Some(mic), Some(speaker)) => {
            ...
            info!("   System Audio: '{}' (via Core Audio tap)", speaker.name);
        }
        (Some(mic), None) => {
            info!("📋 [macOS] Recording device selection complete:");
