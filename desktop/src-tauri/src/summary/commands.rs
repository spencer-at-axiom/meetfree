...
pub async fn api_process_transcript<R: Runtime>(
    ...
    let summary_job = SummaryService::resolve_summary_job(
        &app,
        &pool,
        SummaryJobRequest {
            meeting_id: m_id.clone(),
            text: summary_input_text.clone(),
    ...
    .await
    .map_err(|e| format!("Failed to resolve summary provider config: {}", e))?;

    let runtime = state.summary_runtime.clone();
    tauri::async_runtime::spawn(async move {
        SummaryService::process_transcript_background(app, pool, runtime, summary_job).await;
    });
...
pub async fn api_cancel_summary<R: Runtime>(
    ...
    let cancelled = SummaryService::cancel_summary(&state.summary_runtime, &meeting_id).await;
