...
static SUMMARY_SCHEMA_DOCUMENT: Lazy<Value> = Lazy::new(|| {
    ...
    .unwrap_or_else(|error| {
        log::error!("Summary contract schema JSON is invalid: {}", error);
        Value::Null
    })
});
...
static SUMMARY_SCHEMA_VALIDATOR: Lazy<Option<JSONSchema>> = Lazy::new(|| {
    JSONSchema::options()
        .with_draft(Draft::Draft202012)
        .compile(&SUMMARY_SCHEMA_DOCUMENT)
        .map_err(|error| {
            log::error!("Summary contract schema failed to compile: {}", error);
            error
        })
        .ok()
});
...
impl SummaryPayload {
    ...
    pub fn to_json_value(&self) -> Value {
        serde_json::to_value(self).unwrap_or_else(|error| {
            log::error!("Failed to serialize summary payload: {}", error);
            serde_json::json!({
                "format": "markdown",
                "schema_version": SUMMARY_SCHEMA_VERSION,
                "contract_version": SUMMARY_CONTRACT_VERSION,
...
pub fn validate_summary_payload_value(
    ...
    let validator = SUMMARY_SCHEMA_VALIDATOR.as_ref().ok_or_else(|| {
        ContractValidationError::migration_failed(
            "Summary contract validator is unavailable. The bundled schema failed to initialize.",
        )
    })?;
...
fn collect_keyed_sections(object: &Map<String, Value>) -> Vec<LegacyMarkdownSection> {
    ...
    final_keys.extend(candidate_keys);
...
pub async fn run_startup_migration(
    ...
    let mut report = StartupMigrationReport {
        scanned: rows.len(),
        ..Default::default()
    };
