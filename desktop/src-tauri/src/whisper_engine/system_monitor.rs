...
impl SystemMonitor {
    ...
    pub async fn get_current_resources(&self) -> Result<SystemResources> {
        ...
        let memory_used_percent = if total_memory > 0 {
            ...
            warn!("System monitor reported zero total memory; defaulting memory usage to 0%");
            0.0
        };

        // Get average CPU usage across all cores
        let cpu_usage_percent = if system.cpus().is_empty() {
            warn!("System monitor reported zero CPU cores; defaulting CPU usage to 0%");
            0.0
        } else {
    ...
    pub async fn calculate_safe_worker_count(&self) -> Result<usize> {
        ...
        let memory_based_workers = if self.limits.worker_memory_budget_mb == 0 {
            warn!("Worker memory budget is zero; falling back to a single safe worker");
            1
        } else {
