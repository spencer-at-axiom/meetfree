...
fn recover_lock<'a, T>(mutex: &'a Mutex<T>, name: &str) -> MutexGuard<'a, T> {
    mutex.lock().unwrap_or_else(|poisoned| {
        log::warn!("Recovering from poisoned Whisper command lock: {}", name);
        poisoned.into_inner()
    })
}
...
pub fn set_models_directory<R: Runtime>(app: &AppHandle<R>) {
    let app_data_dir = match app.path().app_data_dir() {
        Ok(path) => path,
        Err(error) => {
            log::error!("Failed to get app data dir for Whisper models: {}", error);
            return;
        }
    };
...
pub async fn whisper_init() -> Result<(), String> {
    let mut guard = recover_lock(&WHISPER_ENGINE, "whisper_engine");
    if guard.is_some() {
        return Ok(());
    }
...
pub async fn whisper_get_available_models() -> Result<Vec<ModelInfo>, String> {
    let engine = {
        let guard = recover_lock(&WHISPER_ENGINE, "whisper_engine");
        guard.as_ref().cloned()
    };
