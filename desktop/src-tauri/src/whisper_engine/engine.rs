...
pub struct WhisperEngine {
    ...
    // Serialize model loading to avoid concurrent WhisperContext initialization races.
...
impl WhisperEngine {
    ...
    pub async fn load_model(&self, model_name: &str) -> Result<()> {
        let _model_load_guard = self.model_load_lock.lock().await;
        ...
        match model_info.status {
            ModelStatus::Available => {
                ...
                let gpu_backend_compiled = Self::detect_gpu_acceleration();
                let effective_use_gpu = adaptive_config.use_gpu && gpu_backend_compiled;
