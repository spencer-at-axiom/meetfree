...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const processBufferedTranscripts = (forceFlush = false) => {
      ...
      const now = Date.now();
      const maxAge = 30000; // CRITICAL FIX: 30 seconds max age to prevent memory leak
      ...
      for (const [sequenceId, transcript] of transcriptBuffer.entries()) {
        ...
        } else if (transcriptAge > maxAge) {
          // CRITICAL FIX: Discard transcripts older than 30 seconds to prevent memory leak
          transcriptBuffer.delete(sequenceId);
          console.warn(
            `🗑️  Discarded stale transcript from buffer: sequence_id=${sequenceId}, age=${transcriptAge}ms (exceeded ${maxAge}ms threshold)`
          );
        } else if (transcriptAge > staleThreshold) {
