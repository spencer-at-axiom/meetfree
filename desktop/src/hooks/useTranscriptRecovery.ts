...
export function useTranscriptRecovery(): UseTranscriptRecoveryReturn {
  ...
  const checkForRecoverableTranscripts = useCallback(async () => {
    ...
    try {
      ...
      // Failed to check for recoverable transcripts - set empty array
    ...
    try {
      ...
      if (folderPath) {
        try {
          ...
          // Audio recovery failed - continue with transcript-only recovery
          audioRecoveryStatus = {
            status: 'failed',
            chunk_count: 0,
      ...
      // Recovery failed - re-throw for caller to handle
      throw error instanceof Error ? error : new Error('Failed to recover meeting');
    } finally {
