#!/bin/bash
# GPU-accelerated development script for Meetfree
...
echo -e "${BLUE}Ã°Å¸Å¡â‚¬ Meetfree GPU-Accelerated Development Mode${NC}"
echo ""
...
if [ -f "package.json" ]; then
    ...
    cd desktop || { echo -e "${RED}Ã¢ÂÅ’ Failed to change to frontend directory${NC}"; exit 1; }
    DESKTOP_DIR="."
else
...
echo ""
echo -e "${BLUE}Ã°Å¸â€œÂ¦ Starting Meetfree in development mode...${NC}"
echo ""
...
if [ -n "$TAURI_GPU_FEATURE" ]; then
    if [ "$TAURI_GPU_FEATURE" == "none" ]; then
        echo -e "${YELLOW}Ã¢Å¡Â Ã¯Â¸Â GPU feature explicitly set to none. Running in CPU-only mode.${NC}"
    else
...
echo ""
echo -e "${BLUE}Ã°Å¸Â¦â„¢ Building meetfree-llm-service sidecar (debug)...${NC}"
...
(cd "$SERVICE_DIR" && cargo build $SERVICE_FEATURES)
...
if [ $? -eq 0 ]; then
    echo ""
    echo -e "${GREEN}Ã¢Å“â€¦ Development server stopped cleanly${NC}"
else
    echo ""
    echo -e "${RED}Ã¢ÂÅ’ Development server encountered an error${NC}"
    exit 1
fi
