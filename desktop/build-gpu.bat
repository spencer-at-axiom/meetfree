@echo off
REM Meetfree GPU-Accelerated Build Script for Windows
...
echo.
echo ========================================
echo   Meetfree GPU-Accelerated Build
echo ========================================
echo.
...
echo Ã°Å¸Â§Â¹ Checking for existing processes on port 3118...
...
echo Ã°Å¸â€Â§ Setting up Visual Studio environment...
if exist "C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvars64.bat" (
    ...
    echo    Ã¢Å¡Â Ã¯Â¸Â  Visual Studio not found, using manual SDK setup
...
echo.
echo Ã°Å¸â€œÂ¦ Building Meetfree...
echo.
...
if exist "package.json" (
    ...
    echo    Ã¢ÂÅ’ Error: Could not find package.json
...
if %USE_PNPM% equ 0 (
    if %USE_NPM% equ 0 (
        echo    Ã¢ÂÅ’ Error: Neither npm nor pnpm found
        exit /b 1
    )
)

REM Detect GPU feature
echo Ã°Å¸â€Â Detecting GPU features...
for /f "delims=" %%i in ('node scripts/auto-detect-gpu.js') do set TAURI_GPU_FEATURE=%%i

if defined TAURI_GPU_FEATURE (
    echo Ã¢Å“â€¦ Detected GPU feature: !TAURI_GPU_FEATURE!
) else (
...
echo.
echo Ã°Å¸Â¦â„¢ Building meetfree-llm-service sidecar (release)...
...
pushd "%SERVICE_DIR%"
call cargo build --release %SERVICE_FEATURES%
if errorlevel 1 (
...
if exist "%SRC_PATH%" (
    ...
    echo Ã¢Å¡Â Ã¯Â¸Â Contents of ..\target\release:
    dir "..\target\release"
    exit /b 1
)
...
echo.
echo Ã°Å¸â€œÂ¦ Building complete Tauri application...
echo.
...
if errorlevel 1 (
    echo.
    echo Ã¢ÂÅ’ Build failed
    exit /b 1
)

echo.
echo ========================================
echo Ã¢Å“â€¦ Build completed successfully!
echo ========================================
echo.
echo Ã°Å¸Å½â€° Complete Tauri application built with GPU acceleration!
echo.
exit /b 0
...
echo.
echo ========================================
echo   Meetfree GPU Build Script - Help
echo ========================================
echo.
echo USAGE:
...
echo   Meetfree with optimal hardware acceleration features:
echo.
