...
echo.
echo ðŸ¦™ Building llama-helper sidecar (debug)...
...
if exist "%SRC_PATH%" (
    ...
    echo âš ï¸ Contents of ..\target\debug:
    dir "..\target\debug"
    exit /b 1
)
...
REM Run tauri dev
echo.
echo ðŸ“¦ Starting complete Tauri application...
echo.
...
if errorlevel 1 (
    echo.
    echo âŒ Development server encountered an error
    exit /b 1
)

echo.
echo ========================================
echo âœ… Development server stopped cleanly
echo ========================================
echo.
exit /b 0
